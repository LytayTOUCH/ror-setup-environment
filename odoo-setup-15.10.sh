...
# Updated & Distributed by: Lytay TOUCH, @2016
#-------------------------------------------------------------------------------
#
# This script will install ODOO Server on
# clean Ubuntu 15.10 Server
#-------------------------------------------------------------------------------
# USAGE:
#
# odoo-install
...
#openerp
OE_USER="odoo"
OE_HOME="/opt/$OE_USER" #/opt/odoo
OE_HOME_EXT="/opt/$OE_USER/$OE_USER-server" #/opt/odoo/odoo-server
...
sudo sed -i s/"#listen_addresses = 'localhost'"/"listen_addresses = '*'"/g /etc/postgresql/$POSTGRESQL/main/postgresql.conf
...
sudo apt-get install wget subversion git bzr bzrtools python-pip node-clean-css node-less -y
...
sudo rm -f wkhtmltox-0.12.2.1_linux-wheezy-amd64.deb
...
#User to cp the existing odoo source code into $OE_HOME_EXT without clonnin from remote
#sudo cp -r ../odoo/ $OE_HOME_EXT
...
sudo ls -s /etc/init.d/$OE_CONFIG /etc/rc2.d/S02$OE_CONFIG
